...
import herbolaria.java.bean.UsuarioDTO;
import herbolaria.java.util.MySqlDbConn;
import java.util.*;
...
public class MySqlUsuarioDAO implements UsuarioDAO {
	...
	public UsuarioDTO validaUsuario(String vusuario) throws Exception {
		...
		if (rs.next()) {
			...
			objUsuario.setCorreo(rs.getString(5));
			objUsuario.setPais(rs.getString(6));
	...
	public void registra(UsuarioDTO objUsuario) throws Exception {
		...
		Connection cn = MySqlDbConn.obtenerConexion();
		// Definimos la sentencia a ejecutar
		String sql = "insert into tbusuario(usuario,clave,nombre,fecnac,correo,pais,telefono) "
				+ " values(?,?,?,?,?,?,?)";
		// La preparamos
		PreparedStatement st = cn.prepareStatement(sql);
		// Asignamos valores a las interrogantes
		...
		st.setString(5, objUsuario.getCorreo());
		st.setString(6, objUsuario.getPais());
	...
	public void modifica(UsuarioDTO objUsuario) throws Exception {
		...
		Connection cn = MySqlDbConn.obtenerConexion();
		// Definimos la sentencia a ejecutar
		String sql = "update tbusuario set clave=?,nombre=?,fecnac=?,correo=?,pais=?,telefono=? where usuario=?";
		// La preparamos
		PreparedStatement st = cn.prepareStatement(sql);
		// Asignamos valores a las interrogantes
		...
		st.setString(4, objUsuario.getCorreo());
		st.setString(5, objUsuario.getPais());
